...
<ul class="snav">
    <li id="sb_upload"<?php if (!isset($_GET['i'])) {echo'class="last_sb"';} ?>>
        <a href="upload.php" <?php check_menu('upload');  ?>>
            <?php i18n('FILE_MANAGEMENT');?>
        </a>
    </li>
    <?php if (isset($_GET['i']) && $_GET['i'] != '') { ?>
        <li id="sb_image" class="last_sb"><a href="#" class="current"><?php i18n('IMG_CONTROl_PANEL');?></a></li>
    <?php } ?>

    <?php exec_action("files-sidebar"); // @hook files-sidebar sidebar list html output  ?>

    <?php if (getDef('GSALLOWUPLOADS', true)) { // allow uploads? ?>
                ...
                <div class="queue-item-wrap">
                    <div class="queue-item dz-preview dz-file-preview">
                        ...
                        <div class="dz-filename">
                            ...
                            <span class="size"> (<span class="dz-size" data-dz-size></span>)</span>
                        </div>
    ...
    <li style="float:right;" id="sb_filesize" class="dispupload">
        <small><?php i18n('MAX_FILE_SIZE'); ?>: <strong><?php echo $fileSizeLimitMB; ?></strong></small>
    </li>

    <?php } // end allow uploads ?>
