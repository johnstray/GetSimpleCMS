...
if (!requestIsAjax() && $SAFEMODE) {
    if (getDef('GSSAFEMODE', true)) {
        doNotify(i18n_r('ER_SAFEMODE'), 'error', true); // cannot disable if set via gsconfig
    } else {
        doNotify(i18n_r('ER_SAFEMODE') . ', <a href="?safemodeoff">' . i18n_r('DISABLE') . '</a>', 'error', true);
    }
}

if (
    !requestIsAjax()
    && file_exists(GSUSERSPATH . _id($USR) . ".xml.reset")
    && get_filename_id() != 'index'
    && get_filename_id() != 'resetpassword'
) {
...
if (isset($_GET['upd'])) {
    $update  = var_in($_GET['upd']); // preset update tokens
}
if (isset($_GET['id'])) {
    $errid   = var_in($_GET['id']);  // preset id argument
}
if (isset($_GET['old'])) {
    $oldid   = var_in($_GET['old']); // preset old id argument
}

if (isset($_GET['success'])) {
    $success = var_in($_GET['success']); // generic success msg
}
if (isset($_GET['error'])) {
    $error   = var_in($_GET['error']);   // generic error msg
}
// if(isset($_GET['err']))  $err     = var_in($_GET['err']); // deprecated not used

if (isset($_GET['updated']) && $_GET['updated'] == 1) {
    $success = i18n_r('SITE_UPDATED'); // RESERVED for update.php only for site upgrades
}
...
switch ($update) {
    ...
    case 'edit-success':
        if (!isset($ptype) && isset($_GET['ptype'])) {
            $ptype = var_in($_GET['ptype']); // preset update tokens
        }
        ...
        if ($ptype == 'edit' && !isset($oldid)) {
            ...
            doNotify(sprintf(i18n_r('ER_YOUR_CHANGES'), $dispid) . '. <a href="backup-edit.php?p=restore&id=' .
                $id . $draftqs . '&nonce=' . get_nonce("restore", "backup-edit.php") . '">' . i18n_r('UNDO') .
                '</a>', 'success', true);
        } elseif ($ptype == 'edit' && isset($oldid)) {
            // page edit changes saved, undo, restore with slug change
            doNotify(sprintf(i18n_r('ER_YOUR_CHANGES'), $dispid) . '. <a href="backup-edit.php?p=restore&id=' .
                $oldid . '&new=' . $id . $draftqs . '&nonce=' . get_nonce("restore", "backup-edit.php") . '">' .
                i18n_r('UNDO') . '</a>', 'success', true);
        } elseif ($ptype == 'restore' && !isset($oldid)) {
            // page has been restored, undo, restore
            doNotify(sprintf(i18n_r('ER_HASBEEN_REST'), $dispid) . '. <a href="backup-edit.php?p=restore&id=' .
                $id . $draftqs . '&nonce=' . get_nonce("restore", "backup-edit.php") . '">' . i18n_r('UNDO') .
                '</a>', 'info', true);
        } elseif ($ptype == 'restore' && isset($oldid)) {
            ...
            doNotify(sprintf(i18n_r('ER_HASBEEN_REST'), $dispid) . '. <a href="backup-edit.php?p=restore&id=' .
                $oldid . '&new=' . $id . $draftqs . '&nonce=' . get_nonce("restore", "backup-edit.php") . '">' .
                i18n_r('UNDO') . '</a>', 'info', true);
        } elseif ($ptype == 'delete') {
            // page has been deleted, undo, restore
            doNotify(sprintf(i18n_r('ER_HASBEEN_DEL'), $errid) . '. <a href="backup-edit.php?p=restore&id=' .
                $errid . '&nonce=' . get_nonce("restore", "backup-edit.php") . '">' . i18n_r('UNDO') .
                '</a>', 'info', true);
        } elseif ($ptype == 'new') {
            // new page has been saved, undo, no restore, delete file
            doNotify(sprintf(i18n_r('ER_YOUR_CHANGES'), $dispid) . '. <a href="deletefile.php?id=' . $id . $draftqs .
                '&nonce=' . get_nonce("delete", "deletefile.php") . '">' . i18n_r('UNDO') . '</a>', 'success', true);
        }
        break;
    ...
    case 'clone-success':
        doNotify(
            sprintf(
                i18n_r('CLONE_SUCCESS'),
                '<a href="edit.php?id=' .  $errid . '">' . $errid .  '</a>'
            ),
            'success'
        );
        break;
    ...
    case 'component-success':
        doNotify(i18n_r('ER_COMPONENT_SAVE') . '. <a href="components.php?undo&nonce=' . get_nonce("undo") . '">' .
            i18n_r('UNDO') . '</a>', 'success', true);
        break;
    case 'comp-restored':
    case 'component-restored':
        doNotify(i18n_r('ER_COMPONENT_REST') . '. <a href="components.php?undo&nonce=' . get_nonce("undo") . '">' .
            i18n_r('UNDO') . '</a>', 'success', true);
        break;
    case 'snippet-success':
        doNotify(i18n_r('ER_SNIPPET_SAVE') . '. <a href="snippets.php?undo&nonce=' . get_nonce("undo") . '">' .
            i18n_r('UNDO') . '</a>', 'success', true);
        break;
    case 'snippet-restored':
        doNotify(i18n_r('ER_SNIPPET_REST') . '. <a href="snippets.php?undo&nonce=' . get_nonce("undo") . '">' .
            i18n_r('UNDO') . '</a>', 'success', true);
        break;
    case 'profile-restored':
        doNotify(
            i18n_r('ER_PROFILE_RESTORED') . '. <a href="profile.php?undo&nonce=' . get_nonce("undo") .
            '&userid=' . $userid . '">' . i18n_r('UNDO') . '</a>',
            'success',
            true
        );
        break;
    case 'settings-success':
        doNotify(i18n_r('ER_SETTINGS_UPD') . '. <a href="settings.php?undo&nonce=' . get_nonce("undo") . '">' .
            i18n_r('UNDO') . '</a>', 'success', true);
        break;
    case 'settings-restored':
        doNotify(i18n_r('ER_OLD_RESTORED') . '. <a href="settings.php?undo&nonce=' . get_nonce("undo") . '">' .
            i18n_r('UNDO') . '</a>', 'success', true);
        if (!$dbn) {
            break;
        }
    ...
    default:
        if (isset($error)) {
            doNotify('<b>' . i18n_r('ERROR') . ':</b> ' . $error, 'error', true);
        } elseif (isset($_GET['cancel'])) {
            doNotify(i18n_r('ER_CANCELLED_FAIL'), 'error');
        } elseif (isset($_GET['logout'])) {
            doNotify(i18n_r('MSG_LOGGEDOUT'), 'info', true, true);
        } elseif (!empty($err)) {
            doNotify('<b>' . i18n_r('ERROR') . ':</b> ' . $err, 'error', true);
        } elseif (isset($success)) {
            doNotify($success, 'success', false);
        }
        break;
}
...
function doNotify($msg, $type = '', $persist = false, $force = false)
{
    ...
    echo '<div class="updated notify ' . ($type == '' ? '' : 'notify_' . $type . ' ') .
        (!$persist ? 'remove' : 'persist') . '"><p>' . $msg . '</p></div>';
}
