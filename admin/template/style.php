...
if (empty($header_base)) {
    $header_base = $primary_3;
}
...
for ($i = 0; $i < 7; $i++) {
    ...
    foreach ($alphas as $alpha) {
        $labelAlphas .= '.label_' . $i . '_' . floor($alpha * 100) . ' {background-color: ' .
            getRGBA($defaultcolors['label_' . $i], $alpha) . "!important ;}\n";
    }
    $labelAlphas .= "\n";
}
...
if (isset($_GET['s'])) {
    if (in_array('wide', explode(',', $_GET['s']))) {
        ...
        if ($width == '0' or $width == '') {
            $width = 'none'; // allow for no max-width
        }
 ...
 * @return mixed or string (depending on second parameter. Returns False if invalid hex color value)
 */
...
function getRGBA($str, $alpha = '1.0')
{
    ...
    if (preg_match($rgbaPattern, $str, $matches)) {
        return 'rgba(' . implode(',', array_slice($matches, 1, 3)) . ',' . $alpha . ')';
    }
    if (preg_match($rgbPattern, $str, $matches)) {
        return 'rgba(' . implode(',', array_slice($matches, 1)) . ',' . $alpha . ')';
    }
    if (preg_match($hexPattern, $str, $matches)) {
        return 'rgba(' . hex2RGB($str, true) . ',' . $alpha . ')';
    }
