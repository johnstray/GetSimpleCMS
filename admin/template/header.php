...
if (getDef("GSTHUMBSSHOW", true)) {
    $bodyclass .= " forcethumbs";
}
if (getDef("GSPAGETABS", true)) {
    $bodyclass .= " tabs";
}
if (getDef('GSNOSIDEBAR', true) && in_array(get_filename_id(), getDef('GSNOSIDEBAR', false, true))) {
    $bodyclass .= " nosidebar";
}

if (!$SAFEMODE && getDef('GSAJAXSAVE', true)) {
    $bodyclass .= " ajaxsave"; // ajaxsave enabled if GSAJAXSAVE and not SAFEMODE
}

if (!isPage('index')) {
    exec_action('admin-pre-header'); // @hook admin-pre-header backend before header output
}
...
<head>
    ...
    <?php if (!isAuthPage()) { /** phpcs:ignore Squiz.ControlStructures.ControlSignature.NewlineAfterOpenBrace */ ?>
...
foreach ($cm_themes as $theme) {
    ...
    <script type="text/javascript">
        ...
        var htmlEditorConfig = {
            ...
            <?php if (!empty($CKEcontentsCss)) {
                echo "contentsCss : '$CKEcontentsCss',";
            } ?>
            <?php if (!empty($CKEconfigjs)) {
                echo "customConfig : '$CKEconfigjs',";
            } ?>
            <?php if (!empty($CKEstyleSet)) {
                echo "stylesSet : '$CKEstyleSet',";
            } ?>
            height : '<?php echo getGSVar('EDHEIGHT'); ?>',
            baseHref : '<?php echo getGSVar('SITEURL'); ?>'
            <?php if (getGSVar('EDTOOL')) {
                echo ",toolbar: " . returnJsArray(getGSVar('EDTOOL'));
            } ?>
            <?php if (getGSVar('EDOPTIONS')) {
                echo ',' . trim(getGSVar('EDOPTIONS'));
            } ?>
            <?php if (getDef("GSCKETSTAMP", true)) {
                echo ",timestamp : '" . getDef("GSCKETSTAMP") . "'\n";
            } ?>
        };
    ...
    if (!isAuthPage()) {
        exec_action('header'); // @hook header backend before html head closes
    }
