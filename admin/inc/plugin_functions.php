...
function loadPluginData()
{
    ...
    if (!is_frontend()) {
        create_pluginsxml(isPage('plugins'));  // only on backend check that plugin files have not changed, and regen
    }
 ...
 *
 * @todo disabled plugins have a version of 'disabled', should be 0 or null, leaving alone for now for legacy support
 *
 * @since 3.4
 * @uses $live_plugins, $SAFEMODE;
 *
    ...
    foreach ($live_plugins as $file => $en) {
        ...
        if ($en !== 'true' || !file_exists(GSPLUGINPATH . $file) || $SAFEMODE) {
            if ($apilookup) {
                ...
                if (is_object($api_data) && !isset($api_data->cached)) {
                    $cnt++;
                }
...
function plugin_info_update()
{
    ...
    $plugin_info_keys = array_keys($plugin_info);
    foreach ($plugin_info_keys as $key) {
        $cached   = getDef('GSNOPLUGINCHECK', true) || $cnt > $maxapi;
 ...
 * @param boolean  $active default=null, sets plugin active or inactive, default=toggle
 * @return boolean returns $active state, null on errors
 */
function change_plugin($pluginid, $active = null)
{
    // toggles if $active was not specified (null)
    if (is_null($active)) {
        $active = !pluginIsActive($pluginid); // invert
    }

    // update plugin state
    $status = setPluginState($pluginid, $active);
    if (!isset($status)) {
        return; // save failed, do no hooks
    }
...
function setPluginState($pluginid, $state)
{
    ...
    // save string bools
    $live_plugins[$pluginid] = 'false';
    if ($state) {
        $live_plugins[$pluginid] = 'true';
    }
    ...
    // do hooks
    if ($state === true) {
        exec_action('plugin-activate'); // @hook plugin-activate a plugin was activated
    }
    if ($state === false) {
        exec_action('plugin-deactivate'); // @hook plugin-deactivate a plugin was deactivated
    }
 ...
 *
 * @param  string  $pluginid pluginid
 * @return boolean true if plugin is found in live_plugins array
 */
 ...
 * @uses $live_plugins
 *
 * @param object $data pass in xml data instead of using plugins.xml file load
 *
 */
function read_pluginsxml($data = null)
{
    ...
    if (!$data) {
        $data = getXML(GSDATAOTHERPATH . getDef('GSPLUGINSFILE'));
    }
...
function create_pluginsxml($force = false)
{
    ...
    if (file_exists(GSPLUGINPATH) === false) {
        return; // plugin files path issue
    }
...
function createNavTab($tabname, $id, $txt, $action = null, $icon = "")
{
    ...
    echo '<li id="nav_' . $id . '" class="plugin_tab"><a href="load.php?id=' . $id . ($action ? '&amp;' . $action : '')
        . '" ' . ($current ? 'class="current"' : '') . ' >'
        . (empty($icon) ? getIcon("TAB_load") : getIcon("TAB_", $icon)) . " " . $txt . '</a></li>';
}
 ...
 *
 * @since 2.0
 * @uses $plugins, $pluginHooks
 *
 * @param string $action Name of hook to execute
 */
function exec_action($action)
{
    global $plugins,$pluginHooks;
    if (getDef("GSUSELEGACYPLUGINS", true)) {
        return exec_action_legacy($action);
    }

    return exec_hook($plugins, $pluginHooks, $action, 'exec_action_callback');
}
...
function exec_filter($filter_name, $data = array())
{
    ...
    if (getDef("GSUSELEGACYPLUGINS", true)) {
        return exec_filter_legacy($filter_name, $data);
    }
...
{
    if (!pluginhookstat('filter', $hook['hook'])) {
        return $data; // skip
    }
 ...
 *
 * @since 3.4
 * @uses $secfilters, $securityFilters
 *
 * @param string $filter_name id of filter
 * @param string $added_function callable function name
 * @param array  $args arguments for $added_function
 * @param int    $priority order of execution of hook, lower numbers execute earlier
 */
...
function exec_secfilter($filter_name, $result = true)
{
    ...
    $newresult = exec_hook(
        ...
        $securityFilters,
        $filter_name,
        'exec_secfilter_callback',
        $args,
        'exec_secfilter_complete'
    );
 ...
 *
 * @todo  could make active an active call count and allow nested filters by count,
 *        say allow 1 instead of none if it was desirable
 *
 * @since  3.4
 * @uses  $plugincallstats
 *
 * @param  string  $id     hook type id
 * @param  string  $hook   hook name
 * @param  boolean $active mark active
 *
 * @return boolean false if start already flagged
 */
function pluginhookstat($id, $hook, $active = true)
{
    global $plugincallstats;

    if (!isset($plugincallstats[$id])) {
        $plugincallstats[$id] = array();
    }
    ...
    // open call, loop flag
    if (isset($plugincallstats[$id][$hook]['active']) && $plugincallstats[$id][$hook]['active'] === true) {
        return false;
    }
 ...
 *
 * @return array   arguments array with required arguments sliced off
 */
...
function add_hook(
    ...
    # add ref to global plugin hook hash array
    $hook_hash_array[$hook_name][$priority][] = &$hook_array[count($hook_array) - 1];
}
...
{
    foreach ($hook_hash_array[$hook_name] as $prioritykey => $hooks) { // loop priorities
        // loop hook arrays
        foreach ($hooks as $hookkey => $hook) {
            if ($hook['function'] == $hook_function) { // check all hooks for our function
 ...
 * @param string $action Name of hook to execute
 */
function exec_action_legacy($action)
{
    global $plugins;
...
function exec_filter_legacy($script, $data = array())
{
    ...
    foreach ($filters as $filter) {
        ...
        if (
            (isset($filter['filter']) && $filter['filter'] == $script)
            || (isset($filter['hook']) && $filter['hook'] == $script)
        ) {
