        ...
        foreach ($items as $page) {
            ...
            for ($i=0;$i<=$level-1;$i++){
                if ($i!=$level-1) {
                    ...
                    /**
 * @todo: work out a replacement for this utf8_encode() call.
*/
                } else {
                    $dash .= '- '; // inner level
                }
...
function getPagesRow($page,$level,$index,$parent,$children)
{
    ...
    if ($page['title'] == '' ) { $pagetitle       = '[No Title] &nbsp;&raquo;&nbsp; <em>'. $page['url'] .'</em>';
    ...
    if(dateIsToday($page['pubDate'])) { $pagepubdate     = ' <span class="datetoday">'. output_date($page['pubDate']) . '</span>';
    } else { $pagepubdate = '<span>'. output_date($page['pubDate']) . "</span>";
    }
...
function get_pages_menu_dropdown($parentitem, $menu, $level, $id = null, $idlevel = null)
{
    ...
    if (count($items)>0) {
        foreach ($items as $page) {
            ...
            if ($parent == (string)$page['url']) { $sel="selected";
            } else { $sel="";
            }
