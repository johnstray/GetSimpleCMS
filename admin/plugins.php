...
foreach ($plugin_info_sorted as $pluginid => $plugininfo) {
    ...
    if (pluginIsActive($pluginid)) {
        ...
        $cls_Disabled = '';
        $trclass      = 'enabled';
        $icon         = '<a href="plugins.php?set=' . $pluginid . $setNonce .
                        '" title="' . i18n_r('DISABLE') . '">' . getIcon("ICO_plugon") . '</a>';
    } else {
        $cls_Enabled  = '';
        // $cls_Disabled = 'hidden';
        $trclass      = 'disabled';
        $icon         = '<a href="plugins.php?set=' . $pluginid . $setNonce .
                        '" title="' . i18n_r('DISABLE') . '">' . getIcon("ICO_plugoff") . '</a>';
    }
    ...
    if (isset($plugininfo['apipath'])) {
        ...
        if (($pluginver > 0) && version_compare($apiver, $pluginver, '>')) {
            $updatelink  = '<br /><a class="updatelink" href="' . $apipath . '" target="_blank">' .
                           i18n_r('UPDATE_AVAILABLE') . ' ' . $apiver . '</a>';
            $needsupdate = true;
        }
    ...
    if ($pluginver > 0) {
        $table .= '<span><br /><b>' . i18n_r('PLUGIN_VER') . ' ' . $pluginver . '</b> ' .
                  '&mdash; ' . i18n_r('AUTHOR') . ': <a href="' . $plugininfo['author_url'] . '" target="_blank">' .
                  $plugininfo['author'] . '</a></span>';
    }

    $table .= $updatelink . '</td><td class="status" >';
    if (!pluginIsActive($pluginid)) {
        $table .= '<a href="plugins.php?set=' . $pluginid . $setNonce . '" class="toggleEnable ' . $cls_Enabled . '" ' .
                  'style="padding: 1px 3px;" title="' . i18n_r('ENABLE') . ': ' . $plugininfo['name'] . '" >' .
                  i18n_r('ENABLE') . '</a>';
    } else {
        $table .= '<a href="plugins.php?set=' . $pluginid . $setNonce . '" class="cancel toggleEnable ' .
                  $cls_Disabled . '" title="' . i18n_r('DISABLE') . ': ' . $plugininfo['name'] . '" >' .
                  i18n_r('DISABLE') . '</a>';
    }
    $table .= '</td>';
    $table .= "</tr>\n";
...
<?php include('template/include-nav.php'); ?>
    ...
    <main id="maincontent" class="col-lg-9" role="main">
        <div class="main">
            ...
            <?php if ($counter > 0) { ?>
                <table class="table table-striped table-hover edittable">
                    <thead>
                        <tr>
                            <th><?php i18n('PLUGIN_NAME'); ?></th>
                            <th><?php i18n('PLUGIN_DESC'); ?></th>
                            <th><?php i18n('STATUS'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php echo $table; ?>
            ...
            <p class="small text-center mb-0">
                <em>
                    ...
                    <?php i18n('PLUGINS_INSTALLED'); ?>
                    <?php
                    if ($counter == 0) {
                        echo ' - <a href="' . $site_link_back_url . 'extend/" target="_blank" >' .
                        str_replace(array('<em>','</em>'), '', i18n_r('GET_PLUGINS_LINK')) . '</a>';
                    }
                    ?>
                </em>
            </p>
